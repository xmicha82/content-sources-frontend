...
export const getContentList: (page: number, limit: number) => Promise<ContentListResponse> = async (
  page,
  limit,
) => {
  const { data } = await axios.get(
    `/api/content_sources/v1/repositories/?offset=${(page - 1) * limit}&limit=${limit}`,
  );
  return data;
};
